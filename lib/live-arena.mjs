...
export function arenaViewFromState(state, previous = demoArenaView) {
  ...
  const publicHistory = Array.isArray(state.recentPurchases) && state.recentPurchases.length ? state.recentPurchases : state.ranking;
  const recentConquests = publicHistory.length
    ? publicHistory.map((entry, index) => ({
        rank: index + 1,
        username: entry.nickname || `Visitante ${index + 1}`,
        shortcode: entry.shortcode || shortcodeFromUrl(entry.postUrl),
        url: entry.postUrl,
